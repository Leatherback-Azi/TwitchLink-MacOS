...
class DataBase:
    ...
    def setupDownload(self, dataModel, accessToken):
        ...
        if accessToken.dataType == "stream":
            ...
            self.fileDownload = {"downloadType": "stream", "stream": stream, "streamData": streamData,
                                 "resolution": streamData.getResolutions()[0], "fileName": fileName,
                                 "saveDirectory": self.temp.fileSaveDirectory}
            ...
            fileName = self.getSafeFileName(fileName) + ".ts"
            self.fileDownload = {"downloadType": "video", "video": video, "videoData": videoData,
                                 "resolution": videoData.getResolutions()[0], "range": [None, None],
                                 "fileName": fileName, "saveDirectory": self.temp.fileSaveDirectory}
        else:
            clip = dataModel
            ...
            self.fileDownload = {"downloadType": "clip", "clip": clip, "clipData": clipData,
                                 "resolution": clipData.getResolutions()[0], "fileName": fileName,
                                 "saveDirectory": self.temp.fileSaveDirectory}
